(function() {
  ...
  var Planner = function(){
    ...
    */
    this.elaboratePlan = function(type, allPlans, allLemmas, constraints_val){
      ...
      //set all the constraints for this plan
      if(isArray(constraints_val) && constraints_val!=null)
        constraints = constraints_val;
      else if(constraints_val!=null)
        constraints.push(constraints_val);
    ...
    this.runPlan = function (board, data, cb_nextExecution, cb_planCompleted, functionName, callback_return_algorithm){
      //I need a "surrogate" function because "this" generates scope problems.
      __runPlan(board, data, cb_nextExecution, cb_planCompleted, functionName, callback_return_algorithm);
    }


    var __runPlan = function(board, data, cb_nextExecution, cb_planCompleted, functionName, callback_return_algorithm){
      ...
      if(Object.keys(data).length == 0){
        ...
        cb_nextExecution(board, url, requestType, __runPlan, functionName, callback_return_algorithm);
      }
      else{
        ...
        ***************************************************************************/
        ...
        if(areTheConstraintsOvercame(data)){
    ...
    */
    ...
    /*
    * Check if the data not has any conflict with the constraints
    */
    var areTheConstraintsOvercame = function(data){
      var overcame = true;
      for(var i=0; i<constraints.length; i++){
        if(data["@type"] == constraints[i]["responseType"]){
          var value_maybe_constrained = data[constraints[i]["whatProperty"]];
          if(value_maybe_constrained != constraints[i]["value"]){
            overcame = false;
            break;
          }
        }
