...
/*
                if(tokens.length > 3){
                  var iri = "";
                  if(/\(<file:\/\//.test(tokens[2])){ //case (3)
                    //for(var y = 2; y < tokens.length; y++)
                    //  iri += tokens[y].replace(/\(<file:\/\//, '').replace('>','').replace(/"/g,'').replace(').','');
                    iri += tokens[2].replace(/\(<file:\/\//, '').replace('>','').replace(/"/g,'').replace(').','');
                    iri += "/";
                    iri += tokens[3].replace(')', '').replace(/"/g,'').replace(/\.$/,'');
                  }
                  else if(/\(<http:\/\//.test(tokens[2])){ //case (6)
                    iri += tokens[2].replace(/\(</, '').replace('>','').replace(/"/g,'').replace(').','');
                    //iri += tokens[2].replace(/\(<http:\/\//, '').replace('>','').replace(/"/g,'').replace(').','');
                    iri += "/";
                    iri += tokens[3].replace(')', '').replace(/"/g,'').replace(/\.$/,'');
                  }
                  else{ //case (4) => return the same (...) without any changes [nb: it will be manage in the Planner module]
                    iri = "(" + tokens[2] + " " + tokens[3].replace(/\.$/,''); //IT WILL GENERATE AND ERROR! CAUSE THE LACK OF "/"
                  }
