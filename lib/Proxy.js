(function() {
  ...
  var Proxy = function(__dirname){
    ...
    /*
              ****** ERROR! I HAVE TO DO AS PARAMETER not only boardID but also host and port
              ****** So the Parameter should be a board object.
              ****** TODO
    */
    ...
    this.getPlantsInfo = function(boardID){
      ...
      reasoner.generatePlanForFirstUseCase(boardID, files_matching.use_case01.getPlantsInfo, function(){
        parser.readParserFile('use_case01/outputs/parser.n3', function(resulted_plans){
          ...
          executeRequest(resulted_plans.lemmas[resulted_plans.plans[plan_id][num_current_executions]].operation["@id"] , resulted_plans.plans[plan_id], resulted_plans.lemmas, num_current_executions, IsPlanCompleted);
  ...
  var executeRequest = function(path, plan, lemmas, num_current_executions, callback){
          ...
          http.get(options_context, function(resp){
            resp.on('data', function(other_chunk){
              ...
              jsonld.expand(data, function(err, expanded) {
                  ...
                  callback(num_current_executions, lemmas, plan, expanded[0]);
              });
            });
          }).on("error", function(e){
      ...
      if(pathNextURL.numberReturnedUrl == "multiple"){
        ...
        */
        ...
        for(var x=0; x<list_to_invoke.length; x++){
          executeRequest(list_to_invoke[x]["@id"], plan, lemmas, num_executions, IsPlanCompleted);
        }
      }
      else{
        //todo
