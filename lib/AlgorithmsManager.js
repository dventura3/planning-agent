(function() {
  ...
  var AlgorithmsManager = function(_dirname){
    ...
    */
      ...
      proxy.getPlantsInfo(board, function(plants, err){
        ...
        proxy.getSensorsValueOnBoard(board, function(sensors, err){
          ...
          proxy.getHourlyForecastWeather(board, function(hourlyWeather, err){
            ...
            var hourlyWeatherOBJ = convertInToHourlyWeatherOBJ(hourlyWeather);
              ...
              proxy.getActuatorsForEachPlant(board, function(actuators, err){
                ...
                for(var index in actuators){
                  ...
                  if(actuators[index].actuatorType == "IrrigationPump"){
                    ...
                    weighted_average = [currentValue + (0.5 * hourlyWeatherOBJ.hour_0.forecastHumidity) + (0.3 * hourlyWeatherOBJ.hour_1.forecastHumidity) + (0.2 * hourlyWeatherOBJ.hour_2.forecastHumidity)]/4;
                    ...
                    weighted_average = [currentValue + (0.5 * hourlyWeatherOBJ.hour_0.forecastLightIntensity) + (0.3 * hourlyWeatherOBJ.hour_1.forecastLightIntensity) + (0.2 * hourlyWeatherOBJ.hour_2.forecastLightIntensity)]/4;
                    ...
                    weighted_average = [currentValue + (0.5 * hourlyWeatherOBJ.hour_0.forecastTemperature) + (0.3 * hourlyWeatherOBJ.hour_1.forecastTemperature) + (0.2 * hourlyWeatherOBJ.hour_2.forecastTemperature)]/4;
    ...
    var convertInToHourlyWeatherOBJ = function(hourlyWeather){
      var hourlyWeatherOBJ = {};
      for(var i in hourlyWeather){
        hourlyWeatherOBJ["hour_"+i] = hourlyWeather[i];
        hourlyWeatherOBJ["hour_"+i].data = new Date(hourlyWeather[i].timestamp);
        var tmp = new Date(hourlyWeather[i].timestamp);
      }
